...
### 2.5. Spring Boot Validation 라이브러리 설치
...
## 4. 엔티티 (Entity)와 리포지터리(Repository)
...
## 7. 의존성 주입 DI(Dependency Injection)

### 7.1. 생성자 주입(Constructor Injection)
...
### 7.4. Remove "field injection" and use "constructor injection" instead 인텔리제이 경고 메시지
...
## 12. SpringBoot QueryDSL

### 12.1. Querydsl

- 최신 자바 백엔드 기술 Spring JPA
- 쿼리를 자바 코드로 작성하고 문법 오류를 컴파일 시점에 잡아준다
- 동적 쿼리 문제 해결
- 쉡게 작성 가능
- Spring Data JPA + Querydql 조합은 코드 반복 작업을 줄여 준다

### 12.2. Build.gradle

- java 17 이상 사용
- javax 패키지명을 jakarta로 변경
...
```bash
/build/generated/sources/annotationProcessor/java/main/com/napico/sbb/
```
...
## 13. OAuth 2.0 (Open Authorization 2.0)

### 13.1. 작동방식

- 인증을 위한 개방형 표준 프로토콜로, third-party 프로그램에게 리소스 소유자를 대신해 리소스 서버에서 제공하는 자원에 대한 접근 권한을 위임하는 방식으로 작동
- third-party 프로그램(구글, 카카오 등)에게 로그인 및 개인정보 관리에 대한 권한을 위임하여 third-party 프로그램이 가지고 있는 사용자에 대한 리소스를 조회
...
sequenceDiagram
  autonumber
  participant A as User<br/>(Resource Owner)
  participant B as Client<br/>(Application)
  participant C as Authorization Server<br/>(Third-Party)
  participant D as Resource Server<br/>(Third-Party)
  A->>B: 1. 로그인 요청
  B->>A: 2. 소셜 로그인 페이지로 리다이렉트
  A->>C: 3. 소셜 로그인
  activate C
  note right of C: 로그인 확인
  C->>B: 4. 등록된 URL로 리다이렉트<br/>+ Authorization CODE 전달
  deactivate C
  B->>C: 5. Authorization CODE로 접속 요청
  C->>B: 6. CODE 확인 후 Access Token 전달
  B->>D: 7. Access Token 으로 User 정보 요청
  D->>B: 8. Access Token 확인 후 User 정보 전달
  note over B: 후처리
  B->>A: 9. 최종 응답
```

- Spring Security의 OAuth2는 권한 부여 코드 승인 방식(Authorization Code Grant Type)으로 위와 같이 진행

13.2.1. 로그인 요청

- Spring Security에서 기본적으로 제공하는 URL : http://{domain}/oauth2/authorization/{registrationId}
- 시큐리티에서 이미 구현을 해두었고 따로 Controller를 만들지 않아도 된다.

13.2.2. 등록된 URL로 리다이렉트 + Authorization CODE 전달

- Spring Security에서 기본적으로 제공하는 URL : http://{domain}/login/oauth2/code/{registrationId}
- 시큐리티에서 이미 구현을 해두었고 따로 Controller를 만들지 않아도 된다.

13.2.3. 최종 응답 전 후처리

- 로그인한 유저가 처음 가입하는 회원인지, 기존의 등록된 회원인지 검증 후 우리 애플리케이션에 접근할 수 있는 토큰을 발급
  ...
  security:
    oauth2:
      client:
        registration:
          google:
            client-id: # 발급 받은 Client ID ${GOOGLE_CLIENT}
            client-secret: # 발급 받은 Client Secret ${GOOGLE_SECRET}
            scope: # 기본 값 openid,profile,email => profile, email 로 설정 변경. openid가 scope에 있으면 Open Id Provider로 인식
              - profile
              - email

          naver:
            client-id: # 발급 받은 Client ID ${NAVER_CLIENT}
            client-secret: # 발급 받은 Client Secret ${NAVER_SECRET}
          ...
          kakao:
            client-id: # 발급 받은 Client ID ${KAKAO_CLIENT}
            client-secret: # 발급 받은 Client Secret  ${KAKAO_SECRET}
            client-authentication-method: client_secret_post  # 인가 코드로 액세스 토큰을 요청할 때 방법을 정의
            authorization-grant-type: authorization_code
        ...
        provider: # google, github, facebook은 CommonOAuth2Provider에 기본 설정 값이 등록되어 제공되지만 kakao, naver 등은 provider에 필요한 값들을 등록해줘야 한다.
          naver:
            authorization-uri: https://nid.naver.com/oauth2.0/authorize # 소셜 로그인 버튼을 눌렀을 때 이동 할 URL . 소셜 로그인 버튼을 눌렀을 때 이동 할 URL
            token-uri: https://nid.naver.com/oauth2.0/token # 사용자 서비스 서버가 Redirect URI로 전달받은 인가 코드로 액세스 토큰을 요청하는 URI
            user-info-uri: https://openapi.naver.com/v1/nid/me  # 서비스 서버가 발급받은 액세스 토큰으로 사용자 정보를 가져오는 URI
            user-info-authentication-method: header # 사용자 정보를 가져올 때 토큰을 보내는 방법 입니다. header, form, query 중 header 가 기본 값 입니다. 기본 값은 헤더에 액세스 토큰을 담아서 보내는 것
            user-name-attribute: response # Naver 응답 값 resultCode, message, response 중 response 지정

          kakao:
            authorization-uri: https://kauth.kakao.com/oauth/authorize # 소셜 로그인 버튼을 눌렀을 때 이동 할 URL . 소셜 로그인 버튼을 눌렀을 때 이동 할 URL
            token-uri: https://kauth.kakao.com/oauth/token  # 사용자 서비스 서버가 Redirect URI로 전달받은 인가 코드로 액세스 토큰을 요청하는 URI
            user-info-uri: https://kapi.kakao.com/v2/user/me  # 서비스 서버가 발급받은 액세스 토큰으로 사용자 정보를 가져오는 URI
            user-info-authentication-method: header # 사용자 정보를 가져올 때 토큰을 보내는 방법 입니다. header, form, query 중 header 가 기본 값 입니다. 기본 값은 헤더에 액세스 토큰을 담아서 보내는 것
